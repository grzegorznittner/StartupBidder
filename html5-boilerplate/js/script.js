	...
	$(function() {
		...
		var Util = function() {
			...
			self.manualCall = function(objectName, methodName, argArray) {
				...
				if (objectName && self.objectMapper[objectName]
						&& self.objectMapper[objectName][methodName]) {
		...
		var User = function() {
			...
			self.generateUserSnippetHtml = function(i, userjson) {
				...
				if (user.profile
						&& (user.profile.profile_id !== userjson.profile_id)) {
			...
			self.showListingsForProfile = function(profile_id, index) {
				...
				backend.listingsForProfile(profile_id,
						searchListings.displayResultsJson, errorCallback);
			};
			self.showCommentsForProfile = function(profile_id, index) {
				...
				backend.commentsForProfile(profile_id, comments.displayResults,
						errorCallback);
			};
			self.showBidsForProfile = function(profile_id, index) {
				...
				backend.bidsForProfile(profile_id, bids.displayResults,
						errorCallback);
			};
			self.showAcceptedBidsForProfile = function(profile_id, index) {
				...
				backend.bidsForProfile(profile_id, bids.displayAcceptedResults,
						errorCallback);
			};
			self.showPaidBidsForProfile = function(profile_id, index) {
				...
				backend.bidsForProfile(profile_id, bids.displayPaidResults,
						errorCallback);
			};
			self.bindUserSnippet = function(i, userjson) {
				...
				$('#user-search-results-list-item-name-' + i
					...
					profile_id : userjson.profile_id
						...
						'click',
						function() {
							util.hashedCall('user', 'showUserProfile', [
				...
				$('#user-search-results-list-item-listings-' + i).data({
					...
					profile_id : userjson.profile_id
						...
						'click',
						function() {
							util.hashedCall('user', 'showListingsForProfile', [
									$(this).data('profile_id'),
									$(this).data('index') ]);
						});
				$('#user-search-results-list-item-comments-' + i).data({
					...
					profile_id : userjson.profile_id
						...
						'click',
						function() {
							util.hashedCall('user', 'showCommentsForProfile', [
									$(this).data('profile_id'),
									$(this).data('index') ]);
						});
				$('#user-search-results-list-item-bids-' + i).data({
					...
					profile_id : userjson.profile_id
						...
						'click',
						function() {
							util.hashedCall('user', 'showBidsForProfile', [
									$(this).data('profile_id'),
									$(this).data('index') ]);
						});
				$(upvote_item)
				...
				$(upvote_item)
						...
						.bind(
								...
								function() {
									...
									backend.upvoteUser(profile_id, callback, errorCallback);
								});
			};
			return self;
		};
		...
		var Header = function() {
			...
			self.setup = function() {
				...
				$('#header-menu-bar-valuation').unbind().bind(
						'click',
						function() {
							util.hashedCall('searchListings',
									'doValuationSearch');
						});
			...
			self.displayUser = function(userobj) {
				if (userobj.profile && userobj.logout_url) {
					...
					$('#header-menu-name-username').html(
							...
							'click',
							function() {
								util.hashedCall('header', 'showHeaderProfile',
										[ $(this).data('profile_id') ]);
							});
					$(
		...
		var Userbox = function() { // module showing a summary box for the logged in user
			...
			self.setup = function() {
				...
				$('#userbox-bids-paid').bind(
						'click',
						function(e) {
							util.hashedCall('user', 'showPaidBidsForProfile', [
									$('#userbox-profile-id').val(), 'zz' ]); // FIXME - handle error messages
						});
		...
		var SearchListings = function() {
			...
			self.doSearch = function(args) {
				...
				backend.searchListings(args.search_type, keywords,
			...
			self.displayMemberProfile = function(profile) {
				if (profile) {
					...
					$('#searchlistings-profile-snippet').html(
							user.generateUserSnippetHtml('search', profile));
			...
			self.displayListings = function(listings) {
				...
				if (listings && listings.length) {
					for (i = 0; i < listings.length; i++) {
						listing = listings[i];
						resultsHtml += listingClass.generateListingSnippetHtml(
								i, listing);
					}
				} else {
		...
		var SearchUsers = function() {
			...
			self.doSearch = function(args) {
				...
				backend.searchUsers(args.search_type, keywords,
						args.start_index, args.max_results,
						searchUsers.displayResultsJson, errorCallback);
			};
		...
		var Profiles = function() {
			...
			self.showProfile = function(profile_id, errorCallback) {
				backend.getProfile(profile_id, self.displayProfile,
						errorCallback);
			};
			...
			self.displayProfile = function(data, msgClass, msgText) {
				...
				$('#profile-snippet').html(
						user.generateUserSnippetHtml('profile', profile));
				...
				if (user.profile
						&& user.profile.profile_id == profile.profile_id) {
					$('#profile-form-readonly').hide();
					...
					$('#profile-edit-joined-date').html(
							util.formatDate(profile.joined_date));
				...
				if (profile.status == 'active') {
					...
					$('#profile-deactivate-link').show().unbind().data(
							...
							function() {
								...
								backend.deactivateProfile($(this).data(
										'profile_id'), profiles.displayProfile,
										errorCallback);
							});
				} else {
					$('#profile-activate-link').show().unbind().data(
							...
							function() {
								...
								backend.activateProfile($(this).data(
										'profile_id'), profiles.displayProfile,
										errorCallback);
							});
		...
		var ListingClass = function() {
			...
			self.saveListing = function(listing_id) {
				...
				listing.suggested_val = bids.calculateValuation(
			...
			self.displayListing = function(data) {
				...
				if (listing.listing_id) {
					$('#listing-snippet').hide().html(
							listingClass.generateListingSnippetHtml('listing',
									listing)).show();
				...
				if (listing.status === 'new') {
					...
					$('#listing-activate-link').show().unbind().data(
							...
							function() {
								...
								backend.activateListing($(this).data(
					...
					$('#listing-withdraw-link').show().unbind().data(
							...
							function() {
								...
								backend.withdrawListing($(this).data(
										'listing_id'), self.displayListing,
										errorCallback);
							});
					$('#listing-activate-link').hide();
			...
			self.displayUploadField = function(fieldRoot, fieldName,
				...
				if (editable) {
					$(fieldSelRoot + '-add').unbind().bind('click',
							uploader.prepareUploader(fieldRoot, fieldName));
				}
				if (fieldValue) {
			...
			self.recalculateSuggestedValuation = function() {
				...
				var valuation = bids.calculateValuation(suggested_amt,
						suggested_pct, true);
				var validationMsg = bids.validateBid(suggested_amt, 'common',
			...
			self.showListing = function(listing_id, index) {
				...
				backend.getListing(listing_id, self.displayListing,
						errorCallback);
			};
			self.showCommentsForListing = function(listing_id, index) {
				...
				backend.commentsForListing(listing_id, comments.displayResults,
						errorCallback);
			};
			self.showBidsForListing = function(listing_id, index) {
				...
				backend.bidsForListing(listing_id, bids.displayResults,
						errorCallback);
			};
				...
				if (listing.listing_id) {
					$('#content-results-list-item-title-' + i).data({
						...
						listing_id : listing.listing_id
							...
							'click',
							function() {
								util.hashedCall('listingClass', 'showListing',
					...
					$(upvote_item)
							...
							.bind(
									...
									function() {
										...
										backend.upvoteListing(listing_id,
			...
			self.generateListingSnippetHtml = function(i, listing) {
				...
				if (user.profile
						&& !(user.profile.profile_id === listing.profile_id)) {
				...
				resultHtml += '</span> | ';
				resultHtml += '$' + util.addCommas(listing.median_valuation)
						+ ' after ';
		...
		var Comments = function() {
			...
			self.saveComment = function(listing_id, profile_id) {
				...
				var callback = function(comment) {
					...
					backend.commentsForListing(listing_id, callback2,
				...
				backend.addComment(listing_id, profile_id, commentText,
						callback, errorCallback);
			};
			...
			self.displayUserSnippet = function(comments_profile) {
				if (comments_profile) {
					$('#comments-profile-snippet').html(
							user.generateUserSnippetHtml('comments',
									comments_profile));
			...
			self.displayListingSnippet = function(listing) {
				if (listing && listing.listing_id) {
					$('#comments-listing-snippet').html(
							listingClass.generateListingSnippetHtml('comments',
									listing));
			...
			self.bindCommentLinks = function(comments, listing) {
				...
				var deleteComment = function() {
					...
					var profile_callback = function() {
						backend.commentsForProfile(profile_id,
								self.displayResults, errorCallback2);
					};
					var listings_callback = function() {
						backend.commentsForListing(listing_id,
								self.displayResults, errorCallback2);
					};
					var callback = listing_id ? listings_callback
							: profile_callback;
					backend.deleteComment($(this).data('comment_id'), callback,
							errorCallback);
				};
				for ( var i = 0; i < comments.length; i++) {
					...
					if (!user.profile
							|| user.profile.profile_id != comment.profile_id) { // not my profile so I can't delete
						continue;
					}
			...
			self.showCommentListing = function(listing_id, index) {
				...
				backend.getListing(listing_id, listingClass.displayListing,
						errorCallback);
			};
		...
		var Bids = function() {
			...
			self.saveBid = function(listing_id, profile_id, profile_username) {
				...
				var callback = function(bid) {
					...
					if (bid) {
						backend.bidsForListing(bid.listing_id, callback2,
								errorCallback2);
					} else {
						$('#bid-save-message').attr('class', 'attention').html(
								'unable to save bid');
				...
				backend.addBid(listing_id, profile_id, profile_username,
			...
			self.changeBidFunc = function(index, actionType) {
				var changeBid = function() {
					...
					var profile_callback = function() {
						backend.bidsForProfile(profile_id, self.displayResults,
								errorCallback2);
					};
					var listings_callback = function() {
						backend.bidsForListing(listing_id, self.displayResults,
								errorCallback2);
					};
					var callback = listing_id ? listings_callback
							: profile_callback;
					var actionFuncMap = {
						consider: backend.considerBid,
						accept: backend.acceptBid,
						reject: backend.rejectBid,
						withdraw: backend.withdrawBid
					};
			...
			self.displayBidSnippet = function(bids_profile) {
				if (bids_profile) {
					$('#bids-profile-snippet').html(
							user.generateUserSnippetHtml('bids', bids_profile));
			...
			self.displayBidForm = function(listing, profile, profileBid) {
				...
				if (listing.listing_id) {
					$('#bids-listing-snippet').html(
							listingClass.generateListingSnippetHtml('bids',
									listing));
				...
				if (listing.status != 'active') {
					...
					if (profileBid) { /* prefill form with previous bid */
						...
						$('#bid-form-update-date').html(
								util.formatDate(profileBid.bid_date));
						...
						$('#bid-calculated-valuation').html(
								util.addCommas(profileBid.valuation));
					} else { /* new bid, default to suggested numbers */
						self.clearBidFields();
						$('#bid-amt').val(util.addCommas(listing.suggested_amt));
				...
				for ( var i = 0; i < bids.length; i++) {
					...
					$('#bids-item-profile-' + i).data({
						...
						profile_id : bid.profile_id
							...
							'click',
							function() {
								util.hashedCall('bids', 'showBidProfile', [
										$(this).data('profile_id'),
										$(this).data('index') ]);
							});
					$('#bids-item-listing-' + i).data({
						...
						listing_id : bid.listing_id
							...
							'click',
							function() {
								util.hashedCall('bids', 'showBidListing', [
										$(this).data('listing_id'),
										$(this).data('index') ]);
							});
					var boundData = {
